...
export default function EventDetails({ event, isPlaceholder }: EventDetailsProps) {
  // Determine ticket options dynamically
  let ticketOptions: Record<string, { label: string, price: number }> = {};
  
  if (event.ticket_types && event.ticket_types.length > 0) {
      event.ticket_types.forEach((t: any) => {
          // Use name as key, but sanitize it for ID safety if needed, or just use name
          ticketOptions[t.name] = { label: t.name, price: t.price };
      });
      ...
      ticketOptions = {
          standard: { label: 'Standard', price: event.price }
      };
  }

  const optionKeys = Object.keys(ticketOptions);
  const defaultOption = optionKeys[0];

  const [selectedOption, setSelectedOption] = useState(defaultOption);
  const [currentPrice, setCurrentPrice] = useState(ticketOptions[defaultOption]?.price || event.price);
  const [quantity, setQuantity] = useState(1);

  const handleOptionChange = (value: string) => {
    if (ticketOptions[value]) {
      setSelectedOption(value);
      setCurrentPrice(ticketOptions[value].price);
    }
  };
  ...
  // Determine image source - use only event.image_url
  console.log('Event image URL:', event.image_url);
  const imageUrl = event.image_url ? event.image_url : '/logo.svg';
        ...
        <div className="rounded-2xl overflow-hidden border border-white/5 bg-card/30 backdrop-blur-sm animate-fade-in-up">
          ...
          <div className="p-6 md:p-8 grid md:grid-cols-3 gap-8">
            ...
            <div className="space-y-5 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
              ...
              {(Object.keys(ticketOptions).length > 1 || (event.ticket_types && event.ticket_types.length > 0)) && (
                <div>
                  <h3 className="font-semibold text-sm mb-3 text-foreground">Choisissez votre place</h3>
                  <RadioGroup value={selectedOption} onValueChange={handleOptionChange} className="grid grid-cols-3 gap-2">
                    {Object.entries(ticketOptions).map(([key, { label, price }]) => (
                      <div key={key}>
                        ...
                        <Label
                          htmlFor={key}
                          className={cn(
                            "flex flex-col items-center justify-center text-center rounded-xl border border-white/10 bg-white/5 p-3 text-xs font-medium hover:bg-white/10 cursor-pointer transition-all duration-300 h-full",
              ...
              <div className="flex flex-col gap-3 pt-2">
                ...
                <Button size="lg" className="w-full bg-gradient-primary hover:opacity-90 transition-opacity border-0 shadow-glow-sm" asChild>
                  <Link href={`/checkout/${slugify(event.name, event.id)}?type=${selectedOption}&price=${currentPrice}&quantity=${quantity}`}>
